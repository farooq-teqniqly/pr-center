...
```mermaid
stateDiagram-v2
    [*] --> Uninitialized: app start, no app password set
    [*] --> Locked: app start, app password already set
    ...
    Unlocked --> Locked: app process stops
...
- **No auto-lock / idle timeout in v1** — once unlocked, stays unlocked until the app process stops. The decrypted key lives server-side in the Blazor process, shared across browser tabs.
