...
internal static class Argon2KeyDeriver
{
    ...
    /// <exception cref="ArgumentException"><paramref name="password"/> is null or whitespace, or the salt is empty.</exception>
    /// <exception cref="ArgumentNullException"><paramref name="parameters"/> or its salt is null.</exception>
    /// <exception cref="ArgumentOutOfRangeException">A numeric parameter is not positive.</exception>
    public static byte[] DeriveKey(string password, KdfParameters parameters)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(password);
        ArgumentNullException.ThrowIfNull(parameters);
        // Guard the persisted parameters so a corrupt/tampered security row fails
        // with a clear argument error rather than an opaque Argon2 crash.
        ArgumentNullException.ThrowIfNull(parameters.Salt);
        ArgumentOutOfRangeException.ThrowIfZero(parameters.Salt.Length);
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(parameters.MemoryKib);
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(parameters.Iterations);
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(parameters.Parallelism);
