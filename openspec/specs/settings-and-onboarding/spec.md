...
## Purpose
The self-service surface that makes the app usable without touching config files:
first-run setup that sets the app password, an owner tokens table for adding,
replacing, and deleting per-owner tokens, and the poll interval control. Every
control here writes through Core use cases into the vault or application
settings and pokes the refresh trigger -- the screen itself never calls GitHub
and never displays a stored token.
