...
## Purpose
The read-only Blazor Server surface over the published queue snapshot: a lock
gate that picks the screen from app lock state, and an inbox that groups, sorts,
and renders the queue. Everything here is a pure projection evaluated relative to
the user -- the UI performs no derivation, no facts access, no persistence, and
