...
- [ ] 7.1 Register the named `HttpClient` (`IHttpClientFactory`) and the real `GitHubFactsClient` in `PrCenter.Web`'s composition root; DI resolution test updated/extended
- [ ] 7.2 Architecture tests still green (GitHub adapter references Core only; no Persistence reference crept in)
